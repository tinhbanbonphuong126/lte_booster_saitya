...
@section('style')
    ...
    {{--<script type="text/javascript" src="{{ asset("plugins/js-marker-clusterer-gh-pages/examples/data.json") }}"></script>--}}
                        ...
                        <div id="inline-actions" class="">
                          ...
                          <span class="item">Cluster size:
                            <select id="size">
                              ...
                              <option value="80">500</option>
                              <option value="80">800</option>
                              <option value="80">1000</option>
                            </select>
                          </span>
...
@section('script')
    ...
    <script type="text/javascript">
        ...
        @php
            $buar = $bunjouchis->toArray();
            $count =count($buar);
        @endphp

        var data =  @php echo json_encode(["count" => $count, "bunjouchis" => $buar], JSON_PRETTY_PRINT); @endphp;
        ...
        function refreshMap() {
            ...
            for (var i = 0; i < {{ $count }}; ++i) {
                ...
                //Check if location unavailable

                if (data.bunjouchis[i].latitude || data.bunjouchis[i].longitude) {

                    var latLng = new google.maps.LatLng(data.bunjouchis[i].latitude,
                        data.bunjouchis[i].longitude)
                    //console.log(data.bunjouchis[i].latitude + "/" + data.bunjouchis[i].longitude);
                    ...
                    var idBunjouchi = data.bunjouchis[i].id;
                    //console.log(idBunjouchi);

                    // Event Closure Listener for each maker.
                    google.maps.event.addListener(marker, 'click', (function (marker, idBunjouchi, infowindow) {
                        return function () {
                            infowindow.setContent("<a href='{{ route("bunjouchiDetail") }}" + "/" + idBunjouchi + "' target='blank' ><img src='/img/google_maps/map_aisatsu@0.5x.png' /></a>");
                            infowindow.open(map, marker);
                        };
                    })(marker, idBunjouchi, infowindow));
        ...
        function initialize() {
            map = new google.maps.Map(document.getElementById('map'), {
                @if($currentRegion)
                zoom: 12,
                center: new google.maps.LatLng({{ $currentRegion->latitude }}, {{ $currentRegion->longitude }}),
                @else
                zoom: 10,
                center: new google.maps.LatLng(34.340149, 134.143444),
                // center: new google.maps.LatLng(34.340149, 134.043444),
                @endif
